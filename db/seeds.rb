...
30.times.each do |i|
  Reading.create(measurement: pm, sensor: sensor, time: Time.now - (i*15).minutes, value: rand(1500).to_f)
end
...
#   data = line.split(',')
#   Sensor.create(id: data[0].to_i, name: data[0], long: data[2].to_f, lat: data[1].to_f, measurements: ms)
# end
